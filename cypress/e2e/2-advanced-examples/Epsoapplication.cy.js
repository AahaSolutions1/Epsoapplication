...
describe("My EpsoapplicationTest Suite", function(){
    it("My EpsoapplicationTest case", function(){
        ...
        cy.contains('Heads of Administration in EU Delegations').click()
        cy.contains('Apply for EPSO/AST/153/22').invoke('removeAttr','target').click()
        cy.get('input[name="login"]').type('janarthanan')
        ...
        cy.get("select[id='lang_id_3']").select('English')
        cy.get("select[id='lang_id_4']").select('English')
        cy.get("select[id='questions_4']").select('Yes')
        cy.get('select[id="questions_31"]').select('Yes (European Commission)')
        cy.get("select[id='questions_32']").select('as trainee')
        cy.get("select[id='questions_28']").select('Yes')
        cy.get("select[id='questions_29']").select('Yes')
        cy.get("select[id='questions_26']").select('No')
        cy.get("select[id='questions_27']").select('No')
        cy.get("select[id='questions_25']").select('Yes')
        ...
        cy.get('input[id="experience_end_date_1"]').type('02/08/2022')
        cy.get('input[id="experience_month_count_1"]').type('37')
        ...
        cy.get('#experience_end_date_2').type('02/08/2022')
        cy.get('#part_time_yes_2').click()
        cy.get('#experience_month_count_2').type('37')
        ...
        cy.get('#st_percentage_2_857').type('5')
        cy.get('#st_percentage_2_858').type('5')
        cy.get('#st_percentage_2_859').type('4')
        cy.get('#st_percentage_2_860').type('5')
        cy.get('#st_percentage_2_861').type('4')
        cy.get('#st_percentage_2_862').type('3')
        cy.get('#st_percentage_2_863').type('5')
        cy.get('#st_percentage_2_864').type('5')
        ...
        cy.get('#quest_8418_Y').click()
        cy.get('#quest_8419').type('Vous êtes capable de réaliser les missions demandées et vous pouvez obtenir des résultats. Vous tenez la combinaison parfaite entre compétences et expériences. Ce qu’il vous faut montrer au recruteur est : qu’est-ce qui vous démarque des autres candidats ? Quels sont vos qualités uniques ? Il est donc indispensable que vous prépariez votre réponse à l’avance en vous aidant des étapes suivantes.')
        cy.get('#quest_8420_Y').click()
        cy.get('#quest_8421').type('Vous êtes capable de réaliser les missions demandées et vous pouvez obtenir des résultats. Vous tenez la combinaison parfaite entre compétences et expériences. Ce qu’il vous faut montrer au recruteur est : qu’est-ce qui vous démarque des autres candidats ? Quels sont vos qualités uniques ? Il est donc indispensable que vous prépariez votre réponse à l’avance en vous aidant des étapes suivantes.')
        cy.get('#quest_8422_Y').click()
        cy.get('#quest_8423').type('Vous êtes capable de réaliser les missions demandées et vous pouvez obtenir des résultats. Vous tenez la combinaison parfaite entre compétences et expériences. Ce qu’il vous faut montrer au recruteur est : qu’est-ce qui vous démarque des autres candidats ? Quels sont vos qualités uniques ? Il est donc indispensable que vous prépariez votre réponse à l’avance en vous aidant des étapes suivantes.')
        cy.get('#quest_8424_Y').click()
        cy.get('#quest_8425').type('Sur un marché tendu où la concurrence est féroce, prononcer les mots qu’il faut au bon moment peut vous faire décrocher le poste auquel vous aspirez tant. Préparez-vous grâce à nos bonnes réponses aux questions délicates posées en entretien.')
        cy.get('#quest_8426_Y').click()
        cy.get('#quest_8427').type('Sur un marché tendu où la concurrence est féroce, prononcer les mots qu’il faut au bon moment peut vous faire décrocher le poste auquel vous aspirez tant. Préparez-vous grâce à nos bonnes réponses aux questions délicates posées en entretien.')
        cy.get('#quest_8428_Y').click()
        cy.get('#quest_8429').type('Sur un marché tendu où la concurrence est féroce, prononcer les mots qu’il faut au bon moment peut vous faire décrocher le poste auquel vous aspirez tant. Préparez-vous grâce à nos bonnes réponses aux questions délicates posées en entretien.')
        cy.get('#quest_8430_Y').click()
        cy.get('#quest_8431').type('Ce faisant, ils nous invitent à considérer la qualité de vie au travail non comme un ensemble de remèdes à des maux que pourraient générer nos organisations, mais comme une démarche globale visant à éviter que ces maux apparaissent et, au-delà, à créer les conditions pour que la personne puisse se développer dans toutes ses dimensions. Puisse se développer, et non être développée, car placer la personne en situation de faire ses choix elle-même est une condition fondamentale du respect de sa dignité.')
        cy.get('#quest_8432_Y').click()
        cy.get('#quest_8433').type('Ce faisant, ils nous invitent à considérer la qualité de vie au travail non comme un ensemble de remèdes à des maux que pourraient générer nos organisations, mais comme une démarche globale visant à éviter que ces maux apparaissent et, au-delà, à créer les conditions pour que la personne puisse se développer dans toutes ses dimensions. Puisse se développer, et non être développée, car placer la personne en situation de faire ses choix elle-même est une condition fondamentale du respect de sa dignité.')
        cy.get('#quest_8434_Y').click()
        cy.get('#quest_8435').type('Avec cette question, l’employeur vérifie dans quoi vous excellez pour mesurer comment vos forces pourront lui être utiles», indique Annie Chauvette, conseillère en main-d’œuvre pour le Centre d’intervention des Basses-Laurentides pour l’emploi.')
        cy.get('#quest_8436_Y').click()
        cy.get('#quest_8437').type('Avec cette question, l’employeur vérifie dans quoi vous excellez pour mesurer comment vos forces pourront lui être utiles», indique Annie Chauvette, conseillère en main-d’œuvre pour le Centre d’intervention des Basses-Laurentides pour l’emploi.')
        cy.get('#save_next_button').click()
